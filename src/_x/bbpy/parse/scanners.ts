...
export namespace Scanner {
  ...
  interface StateI {
    ...
    // TODO reform this 
    afterNextBracket()     : false | MatchedValueAndAfterhandScannerState<PureConstruct.KeyWord   , DerivedOfState<this>> ;
          ...
          const mainGetStateAt: {
            ...
            const this1: XScannerState = {
              ...
              afterNextToken() {
                ...
                for (const [match, nextState] of FOR_TRUTHY(this1.afterNextBracket() ) ) {
                  return [{ type: "bracket", value: match, ...spe(nextState), }, nextState] ;
                }
              ...
              afterNextBracket() {
                for (const [{ fullMatchedGroupsList: match, } ,nextState] of FOR_TRUTHY((
                  this1.afterNextLexUnitWithPattern(/[\{\}\[\]\(\)\<\>\,\;]/, { singleLineOnly: true }) 
                )) ) {
              ...
              afterNextKeyWord() {
                ...
                // TODO move this into proper place
                for (const [{ fullMatchedGroupsList: match, } ,nextState] of FOR_TRUTHY((
                  this1.afterNextLexUnitWithPattern(/(?=[0-9\+\-])[\+\-]?[0-9\.]+/, { singleLineOnly: true }) 
                )) ) {
                  return [match[0] ,nextState] satisfies [string, Scanner.State] ;
                }
                return false ;
...
/** 
 * only meant for demo or tests.
 * 
 * the returned Iterable can be (re)iterated multiple times as wished.
 * 
 * @deprecated only meant for demo or tests.
 * 
 */
export const tokeniseAdHocSourceFile = (
  ((...[
    { value: sourceCode, type, } ,
  ]) => {
    ;
    ...
    return {
      *[Symbol.iterator]() {
        LOOP:
        for (var nextV: Scanner.State | false = s2215 ; nextV; {}) {
          const v = nextV;
          const vAfterNextToken = ((): Scanner.State => v)().afterNextToken() ;
          if (vAfterNextToken) {
            yield vAfterNextToken[0] ;
            nextV = vAfterNextToken[1] ;
            continue LOOP ;
          } else {
            return ;
          }
