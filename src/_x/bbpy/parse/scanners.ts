        ...
        (sourceFileInfDict) => {
          ...
          const mainGetStateAt: {
            ...
            const this1: XScannerState = {
              ...
              afterNextLinebreak() {
                for (const m of FOR_TRUTHY((
                  // TODO
                  sourceCode.slice(presentlyLetterIndex, presentlyLetterIndex + 11 )
                  .match(/^\r?\n/g)
                )) ) {
                  const matchedStr = (
                    (m[0] || BBP.Debug.fail() )
            ...
            const getLookaheadStateAndEtcAtPresent = (
              (options: (
                ReturnType<{ 
                  (...[{ presentlyLetterIndex, ...otherOptions }] : Parameters<typeof getLookaheadStateAndEtcAt> ): typeof otherOptions ; 
                }>
              ) ) => (
