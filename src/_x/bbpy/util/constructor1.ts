...
namespace ClassPseudoConstructor {
  ...
  interface ForContravarImpl<in A> {
    /* made this not the main call signature, to avoid the resulting conflict with that of `ForCovarImpl` */
    process(...args: [never, "contravar", 2, 2, A,]): void ;
  };
  ...
  export type InstanceOf<C extends { (...a: any): any ; }, accessMode extends IofwAccessType> = (
    ...
    | (
      ...
      // )
      // {
      //   [IofwAccessType.W]: ConformanteWhenPossible<C> ;
      //   [IofwAccessType.R]: ReturnValueWhenPossible<C> ;
      // }[IofwAccessType]
      (
        accessMode extends IofwAccessType.W ? ConformanteWhenPossible<C> :
        accessMode extends IofwAccessType.R ? ReturnValueWhenPossible<C> :
  ...
  export type ReturnValueWhenPossible<C extends { (...a: any): any ; }> = (
    ...
    // & (((C | ForCovar<never>) & {}) extends (ForCovar  <infer Instance> ) ? Instance : unknown )
  ) ;
  /** 
   * extract `Instance` from the given desc type
   */
  export type ConformanteWhenPossible<C extends { (...a: any): any ; }> = (
    // | (C extends { (...args: [(infer Instance), ...unknown[]]): unknown } ? Instance : never )  // outdated ; now is behind a name, see the `ForContravar` def above
